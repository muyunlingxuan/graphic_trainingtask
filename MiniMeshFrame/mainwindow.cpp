...
//	connect(action_saveas_, SIGNAL(triggered()), imagewidget_, SLOT(SaveAs()));
	...
	action_miniMesh_ = new QAction(tr("miniMesh"), this);
	action_miniMesh2_ = new QAction(tr("miniMesh2"), this);
	...
	connect(action_miniMesh_, SIGNAL(triggered()), renderingwidget_, SLOT(MiniMesh()));
	connect(action_miniMesh2_, SIGNAL(triggered()), renderingwidget_, SLOT(MiniMesh2()));
}
...
void MainWindow::CreateToolBars()
{
	...
	toolbar_basic_->addAction(action_miniMesh_);
	toolbar_basic_->addAction(action_miniMesh2_);
}
