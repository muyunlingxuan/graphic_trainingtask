...
**/
void WarpingRBF::doWarp(cv::Mat *imageIn) {
...
*/
cv::Point2d WarpingRBF::f_func(cv::Point2d p) {
...
cv::Mat WarpingRBF::calculateDistance_cpoint() {
	int num = vec_points_start_.rows;
	cv::Mat p_mat_ = cv::Mat::zeros(cv::Size(1, 3), CV_32FC1);
	cv::Mat q_mat_ = cv::Mat::zeros(cv::Size(1, 3), CV_32FC1);
	...
	for (int i = 0; i < vec_points_start_.rows; i++) {
		...
		((cv::Mat)((q_mat_ - p_mat_).t())).copyTo(d_cpoint_mat_.row(i));
	}
...
void WarpingRBF::calculateMatrixAi(const cv::Mat &Rd, const cv::Mat &d_cpoint) {
