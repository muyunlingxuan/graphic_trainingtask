...
void Mesh3D::SetBoundaryFlag(void)
{
	for (EDGE_ITER eiter = pedges_list_->begin(); eiter!=pedges_list_->end(); eiter++)
	{
		...
		{
			(*eiter)->insert_boundary_flag(BOUNDARY);
			(*eiter)->ppair_->insert_boundary_flag(BOUNDARY);
			(*eiter)->pvert_->insert_boundary_flag(BOUNDARY);
			(*eiter)->ppair_->pvert_->insert_boundary_flag(BOUNDARY);
			(*eiter)->ppair_->pface_->insert_boundary_flag(BOUNDARY);
		}
	}
}
...
void Mesh3D::ComputePerVertexNormal(HE_vert* hv)
{
	...
	if (hv->boundary_flag_ == INNER)
	{
		...
		do 
		{
			...
			{
				/*hv->insert_position(hv->position() * 1.1f);*/
...
void Mesh3D::ResetFaceSelectedTags(int tag)
{
	...
	{
		get_face(i)->insert_selected(tag);
	}
}
...
void Mesh3D::ResetVertexSelectedTags(int tag)
{
	...
	{
		get_vertex(i)->insert_seleted(tag);
	}
}
