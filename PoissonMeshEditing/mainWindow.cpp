...
void MainWindow::CreateActions()
{
	...
	QStringList selectMethods;
	selectMethods << tr("FixedPoints") << tr("EditablePoints");
	combobox_select_ = new QComboBox(this);
	combobox_select_->addItems(selectMethods);
	connect(combobox_select_, SIGNAL(activated(int)), renderingwidget_, SLOT(SelectMode(int)));
	...
	action_reset_ = new QAction(tr("Reset"), this);
	...
	connect(action_reset_, SIGNAL(triggered()), renderingwidget_, SLOT(Reset()));

	action_select_ = new QCheckBox(tr("Select"), this);
	connect(action_select_, SIGNAL(clicked(bool)), renderingwidget_, SLOT(SelectPoint(bool)));
	action_select_->setChecked(false);

	action_edit_ = new QCheckBox(tr("Edit"), this);
	connect(action_edit_, SIGNAL(clicked(bool)), renderingwidget_, SLOT(EditPoint(bool)));
	action_edit_->setChecked(false);

	/*action_select_ = new QAction(tr("Select"), this);
	connect(action_select_, SIGNAL(triggered()), renderingwidget_, SLOT(SelectPoint()));

	action_edit_ = new QAction(tr("Edit"), this);
	connect(action_edit_, SIGNAL(triggered()), renderingwidget_, SLOT(EditPoint()));*/
}
...
void MainWindow::CreateToolBars()
{
	...
	toolbar_basic_->addAction(action_reset_);

	toolbar_basic_->addWidget(action_select_);
	//toolbar_basic_->addAction(action_select_);
	toolbar_basic_->addWidget(combobox_select_);
	toolbar_basic_->addWidget(action_edit_);
	//toolbar_basic_->addAction(action_edit_);
}
