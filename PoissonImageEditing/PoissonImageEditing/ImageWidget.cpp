...
ImageWidget::ImageWidget(ChildWindow *relatewindow)
{
	...
	imageEdit = new PoissonImageEdit();
}
...
void ImageWidget::setMatrixforPoisson()
{
	...
	QImage *image = source_window_->imagewidget_->image();
	QPoint p(xsourcepos, ysourcepos);

	imageEdit->makeMatrix_A(w, h);
	imageEdit->makeMatrix_b(p, image);
}
	...
	if (Qt::LeftButton == mouseevent->button())
	{
		...
		case kPaste:
		{
			...
			*(image_) = *(image_backup_);
			...
			QPoint p_start_(xpos, ypos);

			imageEdit->clone(image_, p_start_);
		}
